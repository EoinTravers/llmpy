...
from dsllmpy.openai_client import (
    OpenAIClient,
    _RateLimiter,
    _retry_async,
    _retry_sync,
...
def test_call_returns_str(client):
    with patch.object(
        client._client.responses, "create", return_value=_make_completion("hello")
    ):
...
def test_call_passes_json_object_format_when_response_format_set(client):
    ...
    with patch.object(
        client._client.responses, "parse", return_value=_make_completion('{"word": "hi"}')
    ) as mock:
        client.call("sys", "usr", response_format=_G)
    _, kwargs = mock.call_args
    assert kwargs.get("text_format") is _G


def test_call_no_response_format_type_sent(client):
    with patch.object(
        client._client.responses, "create", return_value=_make_completion("hi")
    ) as mock:
        client.call("sys", "usr")
    _, kwargs = mock.call_args
...
def test_call_async_returns_str(client):
    ...
    with patch.object(client._async_client.responses, "create", side_effect=_fake):
...
def test_call_async_response_format_returns_model(client):
    ...
    with patch.object(client._async_client.responses, "parse", side_effect=_fake):
...
def test_call_many_per_system_prompt(client):
    ...
    with patch.object(client._async_client.responses, "create", side_effect=_fake):
        results = asyncio.run(client.call_many(["sys1", "sys2"], ["p1", "p2"], progress=False))
...
def test_send_messages_returns_str(client):
    with patch.object(
        client._client.chat.completions, "create", return_value=_make_chat_completion("hello")
    ):
...
def test_send_messages_multi_turn(client):
    ...
    with patch.object(
        client._client.chat.completions, "create", return_value=_make_chat_completion("Alice")
    ) as mock:
...
def test_send_messages_async_returns_str(client):
    async def _fake(**kwargs):
        return _make_chat_completion("hello")
...
def test_should_retry_rate_limit_error():
    assert (
        _should_retry(RateLimitError(message="rate limit", response=MagicMock(), body={})) is True
    )
...
def test_retry_sync_retries_on_retryable_error():
    ...
    with patch("dsllmpy.openai_client.time.sleep"):
...
def test_retry_async_retries_on_retryable_error():
    ...
    with patch("dsllmpy.openai_client.asyncio.sleep", new_callable=AsyncMock):
...
def test_rate_limiter_sleeps_when_limit_reached():
    ...
    async def _run():
        with patch("dsllmpy.openai_client.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
...
def test_embed_single_string_returns_nested_list(client):
    with patch.object(
        client._client.embeddings, "create", return_value=_make_embedding([[0.1, 0.2, 0.3]])
    ):
...
def test_embed_list_returns_one_vector_per_input(client):
    ...
    with patch.object(client._client.embeddings, "create", return_value=_make_embedding(vecs)):
